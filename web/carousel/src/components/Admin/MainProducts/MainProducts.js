...
export default class Products extends Component {
  ...
  showProducts = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let k = 0; k < data[i].parameters.length; k++) {
        let str =
          data[i].parameters[k].name + ": " + data[i].parameters[k].values;
      ...
      services
        .get("/mainProduct/" + data[i]._id, config)
        .then((response) => {
  ...
  deleteProductHandler = (product) => {
    services
      .delete("/mainProduct/" + product.id, config)
      .then((response) => {
        this.getAllMainProducts();
      })
      .catch((error) => {
        ...
        render: (id, record) => (
          <Space size="large">
            <a
              ...
              onClick={() => {
                handleSubmit("Are you sure to delete this main product?", () =>
