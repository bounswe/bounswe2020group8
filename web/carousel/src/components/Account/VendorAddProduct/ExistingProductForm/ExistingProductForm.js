...
class ExistingProductForm extends Component {
  ...
  render() {
    ...
    parameterInputs =
      Object.entries(params[0]).map(([key, val], igKey) =>{
        if (key !== "tags" && key !== "id") {
          return(
            <Form.Item name={['user', key]} initialValue={val} label={key.charAt(0).toUpperCase() + key.slice(1)}>
              <Input  disabled/>
            </Form.Item>
          );
        } else if (key === "id") {
    ...
    return (
      <Form {...layout} className={classes.myReset} name="nest-messages" onFinish={this.props.clicked}
        ...
        <Form.Item name={['user', 'id']} hidden label="ID" initialValue={productId} rules={[{required: true}]} />
        <Form.Item name={['user', 'tags']} label="Tags" rules={[{required: true}]} initialValue={this.props.tags}>
