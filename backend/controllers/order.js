...
exports.updateOrderStatusVendorController = BaseUtil.createController((req) => {
  ...
  return BB.all([
    ...
    .then((results) => BaseUtil.decideErrorExist(results))
    .then(() => OrderService.updateOrderStatusVendorService({ _id, mainOrderID, orderID, status }));
});
...
exports.updateOrderStatusGuestController = BaseUtil.createController((req) => {
  ...
  return BB.all([
    ...
    .then((results) => BaseUtil.decideErrorExist(results))
    .then(() => OrderService.updateOrderStatusGuestService({ mainOrderID, orderID, status }));
});
...
exports.getOrderByMainOrderIdController = BaseUtil.createController((req) => {
  let { mainOrderID } = req.body;
  return BB.all([
    AppValidator.validateIfNullOrEmpty(mainOrderID, Messages.RETURN_MESSAGES.ERR_UNDEFINED),
  ])
    .then((results) => BaseUtil.decideErrorExist(results))
    .then(() => OrderService.getOrderByMainOrderIdService({ mainOrderID }));
});
